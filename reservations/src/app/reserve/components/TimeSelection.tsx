...
export default function TimeSelection({ form }: { form: AnyFormApi }) {
  ...
  const { data: timeSlots = [], isLoading: isLoadingTimeSlots } = useTimeSlots(roomId, day);
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div>
          ...
          <DatePicker
            ...
            onChange={(value) => value && form.setFieldValue('day', value.toDate(getLocalTimeZone()).toISOString())}
        ...
        <div>
          ...
          <div className="max-h-60 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 py-2 px-1">
            ...
            {timeSlots.map((slot) => (
              <section
                key={slot.id}
                onClick={() => slot.available && form.setFieldValue('date', slot.start)}
